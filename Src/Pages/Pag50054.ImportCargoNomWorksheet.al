page 50054 "Import Cargo Nom Worksheet"
{
    // ApplicationArea = All;
    Caption = 'Import Cargo Nomination Worksheet';
    SourceTable = "Import Cargo Nomi Worksheet";
    PageType = Worksheet;
    ...
    actions
    {
        area(Processing)
        {
            ...
            action(Validate)
            {
                ...
                var
                begin
                    Rec.ImportCargoNominationValidate();
                    CurrPage.Update(false);
                end;
            ...
            action("Create")
            {
                Caption = 'Create Nomination Lines';
                Image = Create;
                Promoted = true;
                PromotedCategory = Process;
                ApplicationArea = All;
    ...
    var
        importmanifest: Record "Import Cargo Nomi Worksheet";
        CargoNomination: Record "Cargo Nomination";
    begin
        importmanifest.Reset();
        importmanifest.SetRange("Job File No.", CargoNomination."Cargo Nomination No.");
        if importmanifest.FindFirst() then
            repeat
                CargoNomination.Reset();
                CargoNomination.SetRange("Cargo Nomination No.", importmanifest."Job File No.");
                //manifestline.SetRange("Line No.", VerRec."Manifest Line No.");
                if CargoNomination.FindFirst() then begin
                    CargoNomination."BL No." := importmanifest."BL No.";
                    CargoNomination."Container Id" := importmanifest."COntainer ID";
                    CargoNomination."CBM Tonage" := importmanifest."CBM Tonage";
    ...
    var
        ImportManifestLine, IWS : Record "Import Cargo Nomi Worksheet";
        RowNo: Integer;
        ColNo: Integer;
        LineNo: Integer;
        MaxRowNo: Integer;
    ...
    begin
        ...
        for RowNo := 2 to MaxRowNo do begin
            ...
            if IWS.FindFirst() then
                ...
                JFileNo := NoSeriesMgt.GetNextNo(IMSSetup."Cargo Job File No.", WorkDate(), true);
