report 50110 "Container Stock"
{
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Job File No.") where("Shortcut Dimension 4 Code" = filter('CONTAINER'), Received = const(true));
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('Containers Stock As on' + '  ' + Format(EndDate), false, '', true, false, true, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.NewRow();
    ...
    begin
        TempExcelBuffer.CreateNewBook('Container Stock Status');
        TempExcelBuffer.WriteSheet('Container Stock Status', CompanyName, UserId);
        TempExcelBuffer.CloseBook();
        TempExcelBuffer.SetFriendlyFilename('Container Stock Status');
