report 50170 "Stripped package Stock"
{
    ApplicationArea = All;
    Caption = 'Stripped package Stock';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Job File No.") where("Shortcut Dimension 4 Code" = filter('STRIPPED PACKAGE'), Received = const(true));
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('Stripped Package Stock As on' + '  ' + Format(EndDate), false, '', true, false, true, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.NewRow();
    ...
    begin
        TempExcelBuffer.CreateNewBook('Stripped Package Stock Status');
        TempExcelBuffer.WriteSheet('Stripped Package Stock Status', CompanyName, UserId);
        TempExcelBuffer.CloseBook();
        TempExcelBuffer.SetFriendlyFilename('Stripped Package Stock Status');
