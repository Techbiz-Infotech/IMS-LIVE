report 50138 "SSR Activity"
{
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Job File No.") where(Verified = const(true));
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('SSR Activity Report', FALSE, '', TRUE, FALSE, TRUE, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.NewRow();
    ...
    begin
        TempExcelBuffer.CreateNewBook('SSR Activity Report');
        TempExcelBuffer.WriteSheet('SSR Activity Report', CompanyName, UserId);
        TempExcelBuffer.CloseBook();
        TempExcelBuffer.SetFriendlyFilename('SSR Activity Report');
