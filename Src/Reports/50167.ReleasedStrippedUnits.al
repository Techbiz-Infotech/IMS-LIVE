report 50167 "Stripped Units Released"
{
    ApplicationArea = All;
    Caption = 'Stripped Units Released';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Job File No.") where("Shortcut Dimension 4 Code" = filter('STRIPPED UNIT'), Released = const(true));
    ...
    begin
        ...
        TempExcelBuffer.AddColumn('Stripped Units Released', FALSE, '', TRUE, FALSE, TRUE, '', TempExcelBuffer."Cell Type"::Text);
        TempExcelBuffer.NewRow();
    ...
    begin
        TempExcelBuffer.CreateNewBook('Stripped Units Released Report');
        TempExcelBuffer.WriteSheet('Stripped Units Released Report', CompanyName, UserId);
        TempExcelBuffer.CloseBook();
        TempExcelBuffer.SetFriendlyFilename('Stripped Units Released Report');
