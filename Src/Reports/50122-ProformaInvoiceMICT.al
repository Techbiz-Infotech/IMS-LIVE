report 50122 "Proforma Invoice-MICT"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Src/Reports/Layouts/ProformaInvoiceMICT.rdlc';
    //UsageCategory = ReportsAndAnalysis;
    //ApplicationArea = All;
    Caption = 'Proforma-Invoice MICT';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            { }
            column(CountOfUnits; CountOfUnits) { }
            column(Countofloose; Countofloose) { }
            column(No_; "No.")
            { }
            ...
            begin
                ...
                IF SalesLineRec.FindFirst() then begin
                    ...
                    SalesLineRec.setrange("Document No.", "No.");
                    SalesLineRec.SetRange("Shortcut Dimension 4 Code", 'CONTAINER');
                    ...
                    SalesLineRec.SetCurrentKey("Shortcut Dimension 1 Code");
                    SalesLineRec.SetAscending("Shortcut Dimension 1 Code", true);
                    SalesLineRec.SetRange("Shortcut Dimension 4 Code", 'UNIT');
                    SalesLineRec.setrange("Document No.", "No.");
                    if SalesLineRec.FindSet() then begin
                        repeat
                            if GroupByContainerID <> SalesLineRec."Shortcut Dimension 1 Code" then begin
                                GroupByContainerID := SalesLineRec."Shortcut Dimension 1 Code";
                                CountOfUnits := CountOfUnits + 1;
                            end else
                                GroupByContainerID := SalesLineRec."Shortcut Dimension 1 Code";
                        until SalesLineRec.Next() = 0;
                    ...
                    SalesLineRec.SetCurrentKey("Shortcut Dimension 1 Code");
                    SalesLineRec.SetAscending("Shortcut Dimension 1 Code", true);
                    SalesLineRec.SetRange("Shortcut Dimension 4 Code", 'LOOSE');
                    SalesLineRec.setrange("Document No.", "No.");
                    if SalesLineRec.FindSet() then begin
                        repeat
                            if GroupByContainerID <> SalesLineRec."Shortcut Dimension 1 Code" then begin
                                ...
                                if manifestLineInfo.FindFirst() then
                                    repeat
                                        Countofloose += manifestLineInfo."CBM Tonage";
                                    until manifestLineInfo.Next() = 0;
                            end else
                                GroupByContainerID := SalesLineRec."Shortcut Dimension 1 Code";
                        until SalesLineRec.Next() = 0;
        ...
        dataitem("Sales Line"; "Sales Line")
        {
            ...
            begin
                ...
                if ManifestLineinfo.FindFirst() then;
                ActualStorageStarts := manifestLineInfo."Date Received";
                clear(Freedays);
                SalesLine2.reset;
                ...
                chargIDLine.Reset();
                chargIDLine.SetRange("Charge ID Group Code", "Sales Line"."Charge ID");
                chargIDLine.SetFilter("Free Days", '<>%1', 0);
                if chargIDLine.FindFirst() then
                    Freedays := chargIDLine."Free Days";
                StorageStarts := CalcDate(FORMAT(Freedays) + 'D', ActualStorageStarts);
            end;
        }
    }
    var
