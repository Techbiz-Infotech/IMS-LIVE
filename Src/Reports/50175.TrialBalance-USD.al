report 50175 "Trial Balance by per-USD"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Src/Reports/Layouts/TrialBalancebyper-USD.rdlc';
    ApplicationArea = Basic, Suite;
    Caption = 'USD Trial Balance by Period';
    ...
    dataset
    {
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            begin
                ...
                if ExchRate <> 0 then
                    ColumnValuesAsText[1] := RoundAmount("Net Change" / ExchRate)
                else
                    ColumnValuesAsText[1] := RoundAmount("Net Change");
                ...
                for i := 2 to MaxCount do begin
                    ...
                    if ExchRate <> 0 then
                        ColumnValuesAsText[i] := RoundAmount("Net Change" / ExchRate)
                    else
                        ColumnValuesAsText[i] := RoundAmount("Net Change");
    ...
    begin
        ...
        if ExchRate = 0 then
            Error('Exchange rate Must not be blank');
    end;
    ...
    var
        ...
        Trial_Balance_by_PeriodCaptionLbl: Label 'USD-Trial Balance by Period';
        IndentTxt: Label 'None,0,1,2,3,4,5';
        ExchRate: Decimal;
        TotalAmount1: Decimal;
        AmountUSD: Decimal;
        ExchangeRateText: Text[50];
